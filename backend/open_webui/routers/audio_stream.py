"""
Audio Streaming Router for Real-time Voice Conversations

This module implements WebSocket-based audio streaming with:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    Protocol:
    - Client sends: {"type": "start", "sample_rate": 16000} to initiate
    - Client sends: binary frames (PCM16 mono 16kHz)
    - Client sends: {"type": "stop"} to finish
    - Server sends: {"type": "ready", "session_id": "..."}
    - Server sends: {"type": "speech_start"} when speech detected
    - Server sends: {"type": "processing", "segment_id": N}
    - Server sends: {"type": "final", "segment_id": N, "text": "..."}
    """
    ...
    try:
        ...
        if msg.get("type") != "start" or int(msg.get("sample_rate", 0)) != TARGET_SR:
            await websocket.send_json({
                "type": "error", 
                "code": "bad_config", 
                "message": "Only 16kHz PCM16 supported"
            })
    ...
    try:
        ...
        log.info(f"WebSocket disconnected for session {session.session_id}")
    except Exception as e:
